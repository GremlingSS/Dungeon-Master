			...
			if(!Werepowers)
				switch(Race)
					if("Human","Dwarf","Elf")
						...
						if(Black)
							ICON.Blend(icon('Female.dmi',"BlackHair"),ICON_OVERLAY)
						else
							ICON.Blend(icon('Female.dmi',"Hair"),ICON_OVERLAY)
					if("Dragon","Spider")
					else
						ICON.Blend(icon('Female.dmi',"Shirt"),ICON_OVERLAY)
	if(Beard)
		ICON.Blend(icon('Beards.dmi',Beard),ICON_OVERLAY)
	if(Hair)
		ICON.Blend(icon('Hair.dmi',Hair),ICON_OVERLAY)
...
/mob/proc/TearMeat(mob/Body/BODY)
	if(DropItem(BODY))
...
/mob/proc/MakeGland(LOC = loc)
	...
	if(CarryPartStats(GLAND))
		return(GLAND)

/mob/proc/CarryPartStats(atom/movable/PART)
	if(PART) //Whenever a new part is created (Bodies too), some stats need to be carried to it. This also allows us to better control the values of those stats.
