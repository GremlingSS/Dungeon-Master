...
/mob/proc/VampireBite(mob/TARGET,HITCHANCE=50,INFECT=15) //300 hit chance is the true 100%, as chance is devided by 3 in the case of having lower strength than your target.
	if(TARGET in view(1,src))
		if(density&&Flying==0&&HasTeeth==1&&TARGET.Race!="Vampire"&&TARGET.Wagon==0&&TARGET.Flying==0&&IsMist==0&&TARGET.Race!="TowerCap")
			if(CoolDown("Bite",300)||INFECT==1) //INFECT 1 is autofeed, this line triggers the cooldown so you can't feed directly after an autofeed, but allows autofeed even if on cooldown.
				...
				if(BITE)
					...
					view(src) << "<b><font color=red> [src] tries to grab hold of [TARGET] and bite them, but [TARGET] escapes!"

/mob/proc/VampireInfection(mob/INFECTEDBYSET)
	spawn() //Via Bite, not potion. Mostly just ownership and messages here.
		...
		if(InfectedBy)
			...
			if(OWNER!=NEWOWNER)
				...
				Owner<<"[src] has turned into a vampire! [x],[y],[z]"
...
/mob/proc/VampireMorph() if(!Flying) if(!IsMist)
	if(Morphed == 1)
		...
		if(SavedIcon)
			...
			usr << "[src] seems to be missing a variable, and is unable to morph. This is a glich. Please report it."
	RebuildOverlays()
...
/mob/proc/VampireMistForm()
	...
	if(density)
		...
		if(isturf(loc))
			var/turf/T=loc
			if(T.density)
				Owner << "Can not materialize in solid objects!"
			else
