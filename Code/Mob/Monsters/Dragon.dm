...
/mob/Monsters/Dragon/New(LOC,GENDER = prob(50))
...
/mob/Monsters/Dragon/NPCDragon/New()
	StayLocal()
	ClawSkill += 50
	..()

/mob/Monsters/Dragon/NPCDragon/Fire
	name = "{NPC Dragons} Fire Dragon"
	FirePoints = 1001

/mob/Monsters/Dragon/NPCDragon/Ice
	name = "{NPC Dragons} Snow Dragon"
	IcePoints = 1001

/mob/Monsters/Dragon/NPCDragon/Poison
...
/mob/proc/DragonLayEgg(COST = 1) //Cost is how much Egg Content you need to lay a dragon.
	if(EggContent >= COST)
		if(ismob(Owner))
			var/mob/OWNER = Owner
			var/DRAGONS
			for(var/mob/M in OWNER.UnitList)
				if(M.Race == "Dragon"||M.icon_state == "Dragon")
					DRAGONS += 1
			if(DRAGONS>=20)
		...
		Owner << "[src] needs [COST-EggContent] more egg content to lay an egg."
...
/mob/proc/DragonBreath(mob/TARGET) //Target is unrequired for Magma/Water Dragons.
	...
	if(!TARGET)
		if(ismob(destination))
			if(destination in oview(src,5))
				return(DragonBreath(destination))
	if(SubRace=="Magma"||SubRace=="Water")
		if(ALLOWED!=1)
			ALLOWED=1
			TARGET=null
	if(ALLOWED)
			...
			switch(SubRace)
				if("Ice")
					...
					if(prob(Level))
						...
						if(TARGET.LeftArmHP <= 0)
							TARGET.HasLeftArm = 0
						if(TARGET.RightArmHP <= 0)
							TARGET.HasRightArm = 0
						if(TARGET.LeftLegHP <= 0)
							TARGET.HasLeftLeg = 0
						if(TARGET.RightLegHP <= 0)
							TARGET.HasRightLeg = 0
						TARGET.LimbLoss()
