...
/*
/mob/proc/WerewolfInfection(mob/TARGET)
	spawn()
		if(TARGET.Race=="Vampire"||TARGET.Critter||TARGET.ImmuneToVampire||TARGET.SubRace=="Werewolf"||TARGET.Infects||TARGET.Race=="Spider"||TARGET.Race=="Dragon"||TARGET.InfectedBy)
			return
		...
		if(prob(65))
			if(TARGET.Race == "Human" || TARGET.Race == "Dwarf" || TARGET.Race == "Lizardman" || TARGET.Race == "Goblin" || TARGET.Race == "Orc" || TARGET.Race == "Kobold")
				...
				if(prob(90))
					...
					view(TARGET) << "[TARGET] resists the controlling impulse of the disease and remains in control of their body and mind!"
			else
			...
			view(TARGET) << "[TARGET] has recovered from the vile infection passed to it by the werewolf"
