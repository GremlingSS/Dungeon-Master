...
/mob/Monsters/Critters/ToggleSelect()

/mob/Monsters/Critters/Select()

/mob/Monsters/Critters/Deselect()
	//Saves having to put a if(!Critter) on the selection.

/mob/Monsters/Critters/New()
	if(prob(50))
		Gender = "Male"
	else
		Gender = "Female"

/mob/Monsters/Critters/DblClick()
	if(!Tame)
		return
	if(Owner!=usr)
		return
			...
			if(src in view(1,M))
				if(!M.Wagon)
					...
					if (Result == "Tell Pet To Breed")
						for(var/mob/Monsters/Critters/C in view(1,src))
							BreedWith(C)
					if (Result == "Kill")
						HP = 0
						GoingToDie = 1
...
/mob/Monsters/Critters/TowerCap
	icon = 'Cave.dmi'
	icon_state = "TowerCap"
	Race = "TowerCap"
	DieAge = 25
...
/mob/Monsters/Critters/Mole
	icon = 'Mole.dmi'
	icon_state = "Normal"
	Race = "Mole"

/mob/Monsters/Critters/CaveSpider
	icon = 'Bugs.dmi'
	icon_state = "Normal"
	Race = "CaveSpider"
	WebContent = 10
...
/mob/Monsters/Critters/DeathBeatle
	HasGland = 3
	icon = 'DeathBeatle.dmi'
	icon_state = "Normal"
	Race = "DeathBeatle"

/mob/Monsters/Critters/Deer
	icon = 'Animal.dmi'
	Race = "Deer"
	weightmax = 100

/mob/Monsters/Critters/PoisonSporePlant
	HasGland = 2
	icon = 'Swamp.dmi'
	icon_state = "Plant2"
	DieAge = 1000
...
/mob/Monsters/Critters/PoisonSporePlant/New()
	spawn()
		while(1)
			var/TOLD = null
...
/mob/Monsters/Critters/CarnivorousPlant
	HasGland = 2
	icon = 'Swamp.dmi'
	icon_state = "Plant1"
	Race = "Plant"
...
/mob/Monsters/Critters/CarnivorousPlant/New()
	usr.Strength = rand(1,1)
	usr.Agility = rand(1,1)
	usr.Defence = rand(1,1)
	usr.MAXHP = rand(5,10)
	...
	spawn()
		for(var/mob/Monsters/M in oview(1,src))
			if(prob(75))
				if(prob(50))
					if(M.HasLeftArm)
						if(!M.WearingLeftArm && M.UsesEquipment)
							M.HasLeftArm = 0
							M.LeftArm = "Destroyed"
							M.BloodContent -= 25
							M.BloodLoss()
				...
				else if(M.HasRightArm)
					if(!M.WearingRightArm && M.UsesEquipment)
						M.HasRightArm = 0
...
/mob/Monsters/Critters/Gremlin/New()
	usr.Strength = rand(1,1)
	usr.Agility = rand(1,1)
	usr.Defence = rand(1,1)
