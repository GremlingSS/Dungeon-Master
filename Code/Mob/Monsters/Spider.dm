...
//			CHOICES+="Queen"
		...
		Owner << "[src] needs at least 50 web content to lay an egg."
...
/mob/proc/SpiderAgeGains() //From Scratch! :D Outside of maybe the messages.
	if(Race=="Spider")
		var/POISON_UP
		var/SILK_UP
		var/DAMAGE_UP_MIN //Generally, sharpness of claws? Meh logic doesn't matter.
		var/DAMAGE_UP_MAX //These two vars are also currently unused, they're just here to remind me to impliment them.
		var/DEFENCE_UP
		...
		if(AGE == 4)
			...
			switch(SubRace)
				if("Warrior")
					icon='SpiderWarriorOlder.dmi'
				if("Hunter")
					icon='SpiderHunterOlder.dmi'
				if("Worker")
					icon='SpiderWorkerOlder.dmi'
				if("Queen")
					Owner << "<b><font color=purple>[src] grows an egg sack!"
					icon = 'SpiderQueenOlder.dmi'
		if(SubRace=="Queen")
			if(AGE==9)
				Owner << "<b><font color=purple>[src]'s organs have matured enough so that she may breed."
		switch(SubRace)
			if("Queen")
				if(PoisonDMG>95)
					PoisonDMG=95
