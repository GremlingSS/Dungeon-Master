...
/atom/movable //Some objects hold these vars, but no turfs. So movable atoms it is.
	var/Race = null
	var/SubRace = null
	var/Werepowers = 0
	var/Level = 0
			...
			if(!Flying)
				if(FLY==null||FLY=="Fly")
					if(HasLeftLeg&&HasRightLeg&&HasLeftArm&&HasRightArm)
						...
						if(SAY)
							if(Race != "Gargoyle")
								Owner << "[src] starts to fly!"
							else
								Owner << "[src] starts to fly, they become highly vulnerable to damage!"
							if(ismob(Owner))
								if(Owner:CoolDown("FlyMessage",12000))
									Owner<<"[src] will now automatically attempt to match the z level of their destination when moving."
						return
					else if(SAY)
						Owner << "[src] is in too much pain to fly!"
				...
				switch(Race)
					...
					if("Dragon")
						if(copytext(icon_state,1,7)=="flying")
							icon_state=copytext(icon_state,7)
				Flying = 0
				WingsOut = 1
				weightmax = Old
				Old = null
