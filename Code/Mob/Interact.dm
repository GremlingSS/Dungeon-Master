/mob/verb/Interact()
	for(var/mob/Monsters/M in usr.Selected)
		...
		if (Result == "This Creature")
			...
			menu2 += "Suicide"
			if(M.Race == "Gargoyle" || M.Race == "Dragon" || M.SubRace == "Illithid" || M.Undead)
				menu2 += "Rest"
			if(M.Race == "Orc")
				if(M.Level >= 40)
					menu2 += "Berserk"
			...
			if(M.HasWings)
				if(M.Race == "Angel of Death" || M.Race == "Demon" || M.Race == "Gargoyle" || M.Race == "Vampire")
					menu2 += "Retract Wings"
				menu2 += "Fly"
			if(M.Race == "Spider")
				menu2 += "Rest"
			...
			switch(Result2)
				...
				if("Give Creature To")
					var/CAN=1
					switch(M.Race)
						if("Spider","Svartalfar","SandKing","Devourer","Dragon","Illithid")
							CAN=0
					if(M.Wagon || M.Infects || M.IsRoyal || M.CantBeGiven)
						CAN=0
					if(!CAN) usr << "This unit cannot be given away."
					if(CAN)
						var/T = input("Set Who as Owner?")as null|text
						if(T)
							for(var/Player/S in world)
								if(S.client)
									if(T == S.name)
				...
				if("Retract Wings")
					if(!M.Flying)
						if(M.Race == "Demon" || M.Race == "Gargoyle" || M.Race == "Vampire" || M.Race == "Angel of Death")
							if(M.WingsOut)
								M.WingsOut = 0
							else
								M.WingsOut = 1
				...
				if("Rest")
					if(!M.IsMist)
						...
						M.Owner << "[M] cannot rest while in the form of mist!"
