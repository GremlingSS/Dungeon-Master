...
/mob/proc/AddLeader(obj/Items/Equipment/CROWN)
	if(!IsRoyal)
		if(alert(Owner,"Do you wish to make [src] into your [GenderToLeader(Gender)]?","Set Leader","Yes","No")=="Yes")
			if(!IsRoyal)
				var/mob/Monsters/KING
				var/mob/Monsters/QUEEN
				for(var/mob/Monsters/M in usr.UnitList)
					if(M.IsRoyal)
						switch(GenderToLeader(M.Gender))
							if("King")
								KING = M
							if("Queen")
								QUEEN = M
				if(Gender=="Female")
					...
					if(KING)
						...
						return
				if(CROWN)
					Owner << "[src] places the [CROWN] upon [GenderToTheir(Gender)] head and becomes [GenderToLeader(Gender)] of your Empire!"
				else
					Owner << "[src] has become [GenderToLeader(Gender)] of your Empire!"
				ChangeRoyal()
				name = "[GenderToLeader(Gender)] [name]"
	if(IsRoyal)
		return(1)

/mob/proc/RemoveLeader()
	if(IsRoyal)
		if(alert(Owner,"Do you wish [src] to step down from being the [GenderToLeader(Gender)] of your Empire? You will have to wait a full hour before declaring a new unit as [GenderToLeader(Gender)].","Remove Leader","Yes","No")=="Yes")
			if(IsRoyal)
				for(var/obj/Items/Equipment/Armour/Crown/CROWN in src)
					UnEquipItem(CROWN,MANUAL=1)
				ChangeRoyal()
				if(copytext(name,1,6)=="King ")
					name=copytext(name,6)
				if(copytext(name,1,7)=="Queen ")
					name=copytext(name,7)
