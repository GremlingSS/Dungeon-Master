...
/client/Topic(hr,h[],hs)
	...
	<input type="submit" value="Search for variable" >
			...
			for(var/A in CREATE_PATH)
				html+={"<a href="javascript:Show('[A]')">[A]</a> &nbsp; "}
			html+="<hr>"
			for(var/X in CREATE_PATH)
				for(var/A in typesof(X)-X)
	...
	<h3 align=center>[O.name] ([O.type])</h3>"}
			for(var/X in O.vars)
				...
				if(!(AA in varz))
					var/pos=1
					for(var/XR in varz)
						if(sorttext(AA, XR) != 1){pos++}else break
					varz.Insert(pos,AA)
					varz[AA]=list()
				varz[AA]+=X
			html+="<hr><center>"
			for(var/R in varz)
				html+={"<a href="javascript:Show('[R]')">[R]</a> &nbsp; "}
...
/proc
	DetermineVarType(X)
		return "[islist(X)? "List" : istext(X)? "Text" : isloc(X)? "Atom" : isnum(X)? "Num" : isicon(X)? "Icon" : istype(X,/datum)? "Type (or datum)" : isnull(X)? X==0?"Num" : "(Null)" : "(Unknown)"]"
	DetermineVarValue(variable)
		if(istext(variable))
			return "\"[variable]\""
		if(isloc(variable))
			return "<i>[variable:name]</i> ([variable:type])"
		if(isnum(variable))
