			...
			if(isturf(M.loc))
				...
				if(!TYPE)
					...
					if(M.HasItem(/obj/Items/ores/stone/))
						...
						TYPES += "Ancient"
					if(M.UsesPoison)
						if(M.HasItem(/obj/Items/Bones/Bones/))
							TYPES += "Bone"
					TYPES += "Cancel"
				...
				var/DIR
				var/list/DIRS = new()
				if(M.z == GROUND_LAYER || M.z == UNDERGROUND_LAYER)
					DIRS += "Up"
				if(M.z == GROUND_LAYER || M.z == SKY_LAYER)
					DIRS += "Down"
				DIRS += "Cancel"
				...
				if(isturf(M.loc) && M.z != HELL_LAYER)
					...
					if(LOC2.density || LOC2.opacity)
						if(LOC2.icon_state != "CaveWall")
