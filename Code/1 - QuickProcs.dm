...
/mob/proc/CoolDown(NAME="Basic",TIME = 150) //Can be called without using arguments to have a 15 second generic cooldown.
	...
	if(TIME > 0)
		...
		spawn(TIME)
			CoolDowns -= "[NAME]" //Removes from active cooldowns after TIME.
...
/mob //Correct this later, might be causing excess process on mob creations for non-player mobs too...
	var/tmp/list/Group1 = new()
	var/tmp/list/Group2 = new()
	var/tmp/list/Group3 = new()
	var/tmp/list/Group4 = new()
	var/tmp/list/Group5 = new()
	var/tmp/list/Group6 = new()
	var/tmp/list/Group7 = new()
	var/tmp/list/Group8 = new()
	var/tmp/list/Group9 = new()

	var/tmp/LastQuickButtonPress=null

/mob/verb/Quick(Number as num,Action as text)
	var/list/LIST = new()
	if(Action != "Set")
		switch(Number)
			if(1)
	...
	switch(Action)
		...
		if("Set")
			for(var/mob/Monsters/M in Selected)
				LIST += M
		if("Select")
			if(LastQuickButtonPress == "[Number]Select")
				for(var/mob/Monsters/M in Selected)
					usr.loc = M.loc
					return
			for(var/mob/Monsters/M in Selected)
